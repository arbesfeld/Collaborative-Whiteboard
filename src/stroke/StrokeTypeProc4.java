...
/**
 * 
 * Stroke that implements StrokeType and is unimplemented right now
 *
 */
public class StrokeTypeProc4 implements StrokeType {
    ...
    /**
     * Creates a series of draw commands based on the mouse movement for the branches
     * @param identifier of layer
     * @param canvas to draw on
     * @param color of stroke
     * @param strokeWidth
    ...
    private List<DrawCommand> branchBulder(LayerIdentifier identifier, Drawable canvas, Color color, int x, int y, Vector2 velocity) {
        ...
        for (int j = 0; j < 2 * length; j++) {
            ...
            if (Math.random() > .8) {
                result.addAll(branchBulder(identifier, canvas, color, (int)(x + Math.min(-velocity.x()*j, 20)), (int)(y + Math.min(velocity.y()*j, 20)), new Vector2(-velocity.x(), velocity.y())));
            }
        }
        return result;
    }
    ...
    /**
     * Returns a string of the brush name
     */
    @Override
